...
- Always pass `--tc-file=tests/global_config_multiarch.py`.
...
# arm64 regression
uv run pytest --tc-file=tests/global_config_multiarch.py --cpu-arch=arm64 ...
```

### Framework behavior

The framework applies arch-specific configuration automatically:

- **Node filtering** — `schedulable_nodes` returns only nodes with `kubernetes.io/arch=<arch>`
- **OS matrices** — arm64 supports fewer OS versions (no Windows, limited RHEL versions)
- **DataSource names** — arch-suffixed (`fedora-amd64`, `fedora-arm64`)
- **CPU model** — arm64 uses `host-passthrough`; amd64 uses the common node CPU model
- **Instance type preferences** — arm64 uses arch-suffixed names (e.g., `rhel.10.arm64`)
