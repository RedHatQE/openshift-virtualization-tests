...
def generate_instance_type_os_matrix(os_name: str, preferences: list[str]) -> list[dict[str, dict[str, Any]]]:
    """
    Generate a list of dictionaries representing the instance type matrix for a given OS type.
    ...
    def extract_version(pref: str) -> int:
        if match := re.search(r"\d+", pref):
            return int(match.group())
        return -1

    latest_os = max(preferences, key=extract_version)
    instance_types: list[dict[str, dict[str, Any]]] = []

    for preference in preferences:
        preference_config: dict[str, Any] = {
            PREFERENCE_STR: preference,
            DATA_SOURCE_NAME: preference.replace(".", "" if os_name == "rhel" else "-"),
        }
