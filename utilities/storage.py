...
def assert_volume_ready(vm: virt_util.VirtualMachineForTests, volume_name: str) -> None:
    """Assert that a persisted volume is ready on the VM.

    Args:
        vm: Virtual machine instance to inspect.
        volume_name: Name of the volume expected to be ready.
    """
    ready_volumes = [
        status["name"]
        for status in vm.vmi.instance.status.volumeStatus
        if status.get("name") == volume_name and status.get("reason") == "VolumeReady"
    ]
    assert ready_volumes, f"Volume '{volume_name}' is not ready on VM {vm.name}"
