...
class VeleroBackup(Backup):
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        try:
            if getattr(self, "teardown", True):
                delete_velero_resource(resource=self, client=self.client)
            else:
                LOGGER.info("Skipping Velero delete for %s %s (teardown=False)", self.kind, self.name)
        except Exception:
            LOGGER.exception("Failed to delete Velero %s %s", self.kind, self.name)
        finally:
...
def is_storage_class_support_volume_mode(storage_class_name, requested_volume_mode):
    ...
    """
    # If no specific mode is requested or Filesystem is requested, assume supported.
    if not requested_volume_mode:
        return True
    if isinstance(requested_volume_mode, str) and requested_volume_mode.lower() != "block":
        return True
    ...
    try:
        ...
        claim_property_sets = getattr(storage_profile, "claim_property_sets", None) or []
        for cps in claim_property_sets:
            vm = cps.get("volumeMode") if isinstance(cps, dict) else getattr(cps, "volumeMode", None)
            if isinstance(vm, str) and vm.lower() == "block":
                return True
    except Exception as e:
        LOGGER.exception("Error checking StorageProfile for %s: %s", storage_class_name, e)
    return False
