...
def get_node_audit_log_entries(log, node, log_entry):
    ...
    error_patterns = [
        ...
        re.compile(r"Unhandled Error.*couldn't get current server API group list.*i/o timeout"),
    ]

    lines = subprocess.getoutput(
        f"{OC_ADM_LOGS_COMMAND} {node} {AUDIT_LOGS_PATH}/{log} | grep {shlex.quote(log_entry)}"
    ).splitlines()
    has_errors = any(any(pattern.search(line) for pattern in error_patterns) for line in lines)
    if has_errors:
