...
def wait_until_vm_running(vm: VirtualMachine, timeout=TIMEOUT_5MIN, sleep=5):
...
def wait_for_restored_vm(
    ...
    def try_get_and_wait_running():
        ...
        try:
            vm = VirtualMachine.get(client=admin_client, namespace=namespace_name, name=vm_name)
            return vm
        except ResourceNotFoundError:
            return None
    ...
    assert vm is not None, "Unreachable: vm should be set after TimeoutSampler loop"
...
def wait_for_oadp_phase(resource, terminal_statuses, timeout, sleep=5) -> str:
    """
    Wait for an OADP resource to reach one of the final phases.
    ...
    raise AssertionError("Unreachable: TimeoutSampler exited unexpectedly")
