...
def hco_with_max_hotplug_ratio(hyperconverged_resource_scope_class):
    """
    Set maxHotplugRatio in HCO spec.liveUpdateConfiguration.
    """
    with ResourceEditorValidateHCOReconcile(
        patches={
            hyperconverged_resource_scope_class: {
                "spec": {"liveUpdateConfiguration": {"maxHotplugRatio": CLUSTER_MAX_HOTPLUG_RATIO}}
            }
        },
...
def u1_small_instancetype(unprivileged_client):
    """Get the u1.small cluster instancetype."""
...
@pytest.fixture(scope="class")
def vm_with_u1_small_instancetype(
    namespace,
    unprivileged_client,
    u1_small_instancetype,
    rhel10_data_source_scope_session,
    hco_with_max_hotplug_ratio,
):
    """Create a running VM using u1.small instancetype (1 CPU, 2Gi memory)."""
    with VirtualMachineForTests(
        name="test-max-hotplug-ratio-vm",
