...
def test_data_import_cron_deletion_on_opt_out(
    ...
    volumes_before = get_all_dic_volume_names(client=admin_client, namespace=golden_images_namespace.name)
    ...
    assert volumes_before == volumes_after, (
        "DataImportCron delete should not impact volumes.\n"
        f"Volumes before: {volumes_before}\n"
        f"Volumes after: {volumes_after}\n"
        f"Added: {set(volumes_after) - set(volumes_before)}\n"
        f"Removed: {set(volumes_before) - set(volumes_after)}"
    )
