...
def validate_vmi_network_receive_and_transmit_packets_total(
    ...
    try:
        for sample in samples:
            if sample:
                LOGGER.info(f"Waiting for the metric {metric_dict['metric_name']} to update")
                time.sleep(TIMEOUT_15SEC)
                ...
                # run_ssh_commands adds automatically more 6 to 7 packets we can't catch.
                if math.isclose(int(sample_value), int(metric_packets_value), rel_tol=0.05):
