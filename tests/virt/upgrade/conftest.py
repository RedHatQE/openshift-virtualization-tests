...
def datasources_for_upgrade(local_admin_client, dvs_for_upgrade):
    data_source_list = []
...
@pytest.fixture(scope="session")
def migratable_vms(local_admin_client, hco_namespace, upgrade_namespaces):
    migratable_vms = get_all_migratable_vms(admin_client=local_admin_client, namespaces=upgrade_namespaces)
...
def unupdated_vmi_pods_names(
    local_admin_client, hco_namespace, hco_target_csv_name, eus_hco_target_csv_name, upgrade_namespaces, migratable_vms
):
    ...
    for ns in upgrade_namespaces:
        ...
        check_pod_disruption_budget_for_completed_migrations(
            admin_client=local_admin_client, namespace=ns.name, timeout=TIMEOUT_90MIN
        )
...
@pytest.fixture()
def base_templates_after_upgrade(local_admin_client):
    return get_base_templates_list(client=local_admin_client)


@pytest.fixture(scope="session")
def run_strategy_golden_image_data_source(local_admin_client, run_strategy_golden_image_dv):
    with DataSource(
