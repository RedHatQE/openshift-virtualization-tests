...
@pytest.fixture(scope="class")
def rhel_vm_inflated_memory(vm_with_hotplug_support):
    """Writes random data to /home to inflate the RHEL VM's page cache, ensuring enough dirty memory for post-copy."""
    LOGGER.info(f"Writing random data to {vm_with_hotplug_support.name} /home to inflate memory")
    run_ssh_commands(
        host=vm_with_hotplug_support.ssh_exec,
        commands=shlex.split("dd if=/dev/urandom of=/var/tmp/fill bs=1M count=4096"),
    )
...
@pytest.mark.usefixtures("rhel_vm_inflated_memory")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            {"mode": "Paused", "extra_patch": {"completionTimeoutPerGiB": 4}},
            id="WIN-Paused",
            marks=[pytest.mark.special_infra, pytest.mark.high_resource_vm],
        ),
    ],
