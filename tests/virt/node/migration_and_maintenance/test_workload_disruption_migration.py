...
def assert_expected_migration_mode(vm: VirtualMachineForTests, expected_mode: str) -> None:
    ...
    if migration_state.mode != expected_mode:
        LOGGER.error(f"Migration mode is not {expected_mode}! VMI MigrationState {migration_state}")
...
@pytest.fixture(scope="class")
def rhel_mem_stressed(vm_with_hotplug_support):
    wait_for_ssh_connectivity(vm=vm_with_hotplug_support)
    # Allocate 2GB, dirty all pages at 4KB intervals, sleep for 1 second, and repeat infinitely
    cmd = [
        "/bin/bash",
        "-c",
        "nohup python3 -c 'import time; a = bytearray(2*1024*1024*1024); "
        'exec("while True: [a.__setitem__(i, (a[i]+1)%256) for i in range(0, len(a), 4096)]; time.sleep(0.5)")\' '
        ">/dev/null 2>&1 &",
    ]
    run_ssh_commands(
        host=vm_with_hotplug_support.ssh_exec,
        commands=cmd,
...
@pytest.fixture(scope="class")
def windows_mem_stressed(vm_with_hotplug_support):
    wait_for_ssh_connectivity(vm=vm_with_hotplug_support)
    mem_load_process = (
        "powershell.exe -NoProfile -WindowStyle Hidden -Command "
        '"$a=New-Object byte[] (1GB); while($true){ for($i=0;$i -lt $a.Length;$i+=4096){ $a[$i]++ } }"'
    )
    full_cmd = [
        "powershell",
        "Invoke-WmiMethod",
...
@pytest.mark.usefixtures("rhel_mem_stressed")
class TestRhelWorkloadMigration:
    ...
    @pytest.mark.dependency(depends=[f"{RHEL_CLASS_NAME}::migrate_vm"])
