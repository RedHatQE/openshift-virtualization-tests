...
from tests.utils import assert_guest_os_memory_amount, clean_up_migration_jobs, wait_for_guest_os_cpu_count
...
def migration_mode(request, workload_disruption_migration_policy):
    ...
    with ResourceEditor(
        patches={workload_disruption_migration_policy: {"spec": {"allowPostCopy": mode == "PostCopy"}}}
    ):
        yield mode
...
def vm_background_process_id(vm_with_hotplug_support):
    if OS_FLAVOR_WINDOWS in vm_with_hotplug_support.os_flavor:
...
def migrated_vm_with_hotplug_support(admin_client, vm_with_hotplug_support):
    migrate_vm_and_verify(
        ...
        timeout=TIMEOUT_30MIN if OS_FLAVOR_WINDOWS in vm_with_hotplug_support.os_flavor else TIMEOUT_15MIN,
...
@pytest.fixture()
def drained_node_for_hotplug_vm(admin_client, hco_namespace, vm_with_hotplug_support):
    with drain_node(
