...
from utilities.constants import OS_FLAVOR_WINDOWS, REGEDIT_PROC_NAME, TCP_TIMEOUT_30SEC
...
def assert_same_pid_after_migration(orig_pid: str, vm: VirtualMachineForTests) -> None:
    if OS_FLAVOR_WINDOWS in vm.os_flavor:
...
def start_memory_pressure_on_vm(vm: VirtualMachineForTests) -> None:
    ...
    """
    is_windows = OS_FLAVOR_WINDOWS in vm.os_flavor
    ...
    if is_windows:
        ...
        kill_cmd = ["/bin/bash", "-c", "pkill -f 'python3 -c.*bytearray'"]
