...
@pytest.fixture(scope="class")
def sandbox_validator_pods_before_grid_relabel(admin_client, vgpu_ready_nodes):
    """Capture sandbox-validator pod names on node[1] before grid vgpu.config labeling."""
    return {pod.name for pod in get_sandbox_validator_pods(admin_client=admin_client, nodes=[vgpu_ready_nodes[1]])}


@pytest.fixture(scope="class")
def node_labeled_with_grid_vgpu_config(
    vgpu_ready_nodes, supported_gpu_device, sandbox_validator_pods_before_grid_relabel
):
