...
def non_existent_mdev_bus_nodes(workers_utility_pods, vgpu_ready_nodes):
    ...
    non_existent_mdev_bus_nodes = multiprocessing.Manager().list()
    ...
    with multiprocessing.Pool(processes=min(len(vgpu_ready_nodes), multiprocessing.cpu_count())) as pool:
        pool.map(check_node, vgpu_ready_nodes)

    if non_existent_mdev_bus_nodes:
        pytest.fail(
            reason=(
                f"On these nodes: {list(non_existent_mdev_bus_nodes)} {desired_bus} is not available."
