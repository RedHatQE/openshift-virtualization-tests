...
pc_q35_rhel8_4 = "pc-q35-rhel8.4.0"
...
@pytest.fixture()
def run_kubevirt_api_lifecycle_automation_job(
    request,
...
@pytest.fixture(scope="session")
def kubevirt_api_lifecycle_cluster_role_binding(kubevirt_api_lifecycle_service_account):
...
def vm_with_unschedulable_machine_type(admin_client, namespace):
    with VirtualMachineForTests(
        ...
        image=Images.Rhel.RHEL9_REGISTRY_GUEST_IMG,
        memory_guest=Images.Rhel.DEFAULT_MEMORY_SIZE,
        machine_type="pc-q35-rhel7.4.0",
    ) as vm:
        vm.start(timeout=TIMEOUT_1MIN)
        yield vm
...
def test_nodes_have_machine_type_labels(workers):
    ...
    """
    for node in workers:
        assert any(label.startswith("machine-type.node.kubevirt") for label in node.labels.keys()), (
            f"Node {node.name} does not have 'machine-type' label"
        )
...
def test_vm_scheduling_based_on_machine_type(
    ...
    assert vm_with_unschedulable_machine_type.instance.status.get("printableStatus") == "ErrorUnschedulable", (
...
def test_machine_type_transition(
    admin_client,
    vm_with_schedulable_machine_type,
    machine_type_from_kubevirt_config,
    run_kubevirt_api_lifecycle_automation_job,
):
    params, job = run_kubevirt_api_lifecycle_automation_job
    restart_required = params["restart_required"]
    if restart_required == "true":
        wait_for_running_vm(vm=vm_with_schedulable_machine_type)
        assert vm_with_schedulable_machine_type.vmi.status == VirtualMachineInstance.Status.RUNNING, (
            f"VM {vm_with_schedulable_machine_type.name} should be running after machine type transition"
        )
        assert (
        ...
        assert vm_with_schedulable_machine_type.vmi.instance.spec.domain.machine.type == pc_q35_rhel8_4, (
