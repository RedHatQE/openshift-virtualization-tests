...
def test_cnv_daemonset_sno_one_scheduled(subtests, discovered_cnv_daemonsets):
    for daemonset in discovered_cnv_daemonsets:
        with subtests.test(msg=daemonset.name):
            daemonset_instance = daemonset.instance
            ...
            base_error_message = f"For daemonset: {daemonset.name}, expected: 1, "
