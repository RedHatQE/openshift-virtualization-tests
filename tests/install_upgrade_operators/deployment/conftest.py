...
@pytest.fixture()
def deployment_by_name(request, local_admin_client, hco_namespace):
    """
...
def cnv_deployments_excluding_hpp_pool(local_admin_client, hco_namespace):
    return [
        deployment
        for deployment in get_deployments(admin_client=local_admin_client, namespace=hco_namespace.name)
