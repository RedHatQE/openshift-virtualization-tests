...
TEST_CLASS_NAME = "TestHCODefaultCPUModelSet"
...
@pytest.fixture(scope="class")
def fedora_vm_scope_class(unprivileged_client, namespace):
...
class TestHCODefaultCPUModelSet:
    ...
    def test_default_value_for_cpu_model(
        ...
        assert_vmi_cpu_model(
            vmi_resource=fedora_vm_scope_class,
            expected_cpu_model=default_vmi_cpu_model,
        )

    @pytest.mark.polarion("CNV-9025")
    @pytest.mark.dependency(name=f"{TEST_CLASS_NAME}::test_set_hco_default_cpu_model")
    ...
    @pytest.mark.dependency(
        name=f"{TEST_CLASS_NAME}::test_set_hco_default_cpu_model_with_existing_vm",
        depends=[f"{TEST_CLASS_NAME}::test_set_hco_default_cpu_model"],
    )
    def test_set_hco_default_cpu_model_with_existing_vm(
        ...
        fedora_vm_scope_class.restart(wait=True)
        assert_vmi_cpu_model(
            vmi_resource=fedora_vm_scope_class,
            expected_cpu_model=updated_vmi_cpu_model,
        )

    @pytest.mark.polarion("CNV-9027")
    @pytest.mark.dependency(depends=[f"{TEST_CLASS_NAME}::test_set_hco_default_cpu_model_with_existing_vm"])
    def test_hco_default_cpu_model_vm_migration(
        self,
        fedora_vm_scope_class,
        updated_vmi_cpu_model,
        schedulable_nodes,
        ...
        """
        When HCO defaultCPUModel is set, it should be preserved during
        VM migration. After VM migration completes, the VMI should still
        reflect the correct CPU model as configured in HCO
        """
        if len(schedulable_nodes) >= 2:
            migrate_vm_and_verify(vm=fedora_vm_scope_class)
            assert_vmi_cpu_model(
                vmi_resource=fedora_vm_scope_class,
                expected_cpu_model=updated_vmi_cpu_model,
            )
