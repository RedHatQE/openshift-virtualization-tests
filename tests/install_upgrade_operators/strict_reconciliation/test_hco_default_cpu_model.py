...
def get_hco_virtual_machine_options(hco_resource):
    return hco_resource.instance.to_dict()["spec"].get("virtualization", {}).get("virtualMachineOptions", {})


def assert_updated_hco_default_cpu_model(hco_resource, expected_cpu_model):
    hco_cpu_model = get_hco_virtual_machine_options(hco_resource=hco_resource).get(HCO_DEFAULT_CPU_MODEL_KEY)
