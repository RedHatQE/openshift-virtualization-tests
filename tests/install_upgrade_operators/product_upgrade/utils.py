...
def get_all_kubevirt_alerts(alert_dict: dict[str, Any]) -> list[dict[str, dict[str, str]]]:
    return [
        alert
        for alert in alert_dict["data"].get("alerts")
        if alert["labels"].get("kubernetes_operator_part_of") == "kubevirt"
    ]


def get_all_cnv_alerts(prometheus: Prometheus, file_name: str, base_directory: str) -> list[dict[str, dict[str, str]]]:
    cnv_alerts = get_all_kubevirt_alerts(alert_dict=prometheus.alerts())
    write_to_file(
...
def wait_for_pending_alerts_to_fire(prometheus: Prometheus, pending_alerts: list[str]) -> None:
    def _get_non_fired_pending_alerts(_prometheus: Prometheus, _alert_list: list[str]) -> list[str]:
        current_pending_alerts_names = [
            ...
            for _alert in get_all_kubevirt_alerts(_prometheus.alerts())
