...
from tests.install_upgrade_operators.golden_images.multi_arch.utils import (
    get_arch_annotated_resources_dict,
)
...
def arch_annotated_resources_dict(admin_client, golden_images_namespace, worker_nodes_architectures):
    return get_arch_annotated_resources_dict(
...
def expected_resources_with_arch_suffix(
    ...
    resources_dict = {
        resource_kind: {
            f"{resource_name}-{arch}" for resource_name in resource_names for arch in worker_nodes_architectures
        }
        for resource_kind, resource_names in default_common_templates_related_resources.items()
        if resource_kind in (DataImportCron.kind, DataSource.kind)
    }
...
class TestEnableMultiArchImageImport:
    ...
    def test_arch_annotated_resources(
        ...
        for resource_type in [DataImportCron, DataSource]:
            ...
            ready_condition = "UpToDate" if resource_kind == DataImportCron.kind else DataSource.Condition.READY
