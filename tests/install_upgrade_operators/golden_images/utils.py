...
def verify_resource_in_ns(
    ...
    """
    resources = resource_list or resource_type.get(dyn_client=dyn_client, namespace=namespace)
    resources_names = {resource.name for resource in resources}
    assert_sets_equal(
        actual=resources_names,
        expected=expected_resource_names,
        context=f"{resource_type.kind} resources in {namespace} mismatching names",
    )
...
def verify_data_import_cron_template_annotation(template: dict[str, Any], expected_architectures: set[str]) -> None:
    actual_architectures = set(template["metadata"]["annotations"].get("ssp.kubevirt.io/dict.architectures").split(","))
    assert_sets_equal(
        actual=actual_architectures,
        expected=expected_architectures,
        context=f"Template {template['metadata']['name']} mismatching architectures",
    )
