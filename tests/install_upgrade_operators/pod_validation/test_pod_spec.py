...
def test_pods_priority_class_value(
    ...
    if any(pod.name.startswith(VIRT_PLATFORM_AUTOPILOT) for pod in cnv_pods_by_type) and jira_88756_open:
        pytest.xfail(f"{VIRT_PLATFORM_AUTOPILOT} pod has no priority class name due to CNV-88756 bug")
