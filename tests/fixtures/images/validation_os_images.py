...
@pytest.fixture(scope="session")
def windows_validation_os_images_persistent_volume_claim_scope_session(
    validation_os_images_role_binding, conformance_tests
):
    """Provides the PVC backing the Windows Server 2022 image in the validation-os-images namespace.

    Resolution order:
        1. DataVolume exists — waits for success, yields its PVC.
        2. PVC exists without a DataVolume — user-provided image, yields the PVC directly.
        3. Neither exists — imports via Artifactory (fails on conformance runs), yields the new PVC.

    Yields:
        PersistentVolumeClaim: The PVC containing the Windows 2022 image.
    """
...
def windows_validation_os_images_data_source_scope_session(
    admin_client, windows_validation_os_images_persistent_volume_claim_scope_session
):
    ...
    if win_data_source.exists:
        ...
        assert source_pvc.name == windows_validation_os_images_persistent_volume_claim_scope_session.name, (
            f"DataSource {win_data_source.name} source PVC name is {source_pvc.name}, "
            f"expected {windows_validation_os_images_persistent_volume_claim_scope_session.name}"
        )
        assert source_pvc.namespace == windows_validation_os_images_persistent_volume_claim_scope_session.namespace, (
            f"DataSource {win_data_source.name} source PVC namespace is {source_pvc.namespace}, "
            f"expected {windows_validation_os_images_persistent_volume_claim_scope_session.namespace}"
        )
