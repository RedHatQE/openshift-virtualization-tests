...
def hotplugged_dvs_scope_class(
    ...
    with ExitStack() as stack:
        for idx, dv in enumerate(blank_dvs_multi_storage_for_hotplug_scope_class):
            disk_serial = f"{serial_base}-{idx}" if serial_base else None
...
class TestHotPlugWithSerialPersist:
    ...
    def test_hotplug_volume_with_serial_and_persist(
        ...
        expected_serials = [f"{HOTPLUG_DISK_SERIAL}-{idx}" for idx in range(len(hotplugged_dvs_scope_class))]
        assert_disk_serial(vm=fedora_vm_for_hotplug_scope_class, serials=expected_serials)
        assert_hotplugvolume_nonexist(vm=fedora_vm_for_hotplug_scope_class)
    ...
    def test_hotplug_volume_with_serial_and_persist_migrate(
        ...
        if all(is_dv_migratable(dv=dv) for dv in hotplugged_dvs_scope_class):
            ...
            for data_volume in hotplugged_dvs_scope_class:
                assert_volume_ready(vm=fedora_vm_for_hotplug_scope_class, volume_name=data_volume.name)
            expected_serials = [f"{HOTPLUG_DISK_SERIAL}-{index}" for index in range(len(hotplugged_dvs_scope_class))]
            assert_disk_serial(vm=fedora_vm_for_hotplug_scope_class, serials=expected_serials)
        else:
            LOGGER.warning(
    ...
    def test_hotplug_volume_with_serial_and_persist_after_reboot(
        ...
        Expected:
            - All disk serials are visible after reboot
        """
        ...
        for dv in hotplugged_dvs_scope_class:
            assert_volume_ready(vm=fedora_vm_for_hotplug_scope_class, volume_name=dv.name)
        expected_serials = [f"{HOTPLUG_DISK_SERIAL}-{idx}" for idx in range(len(hotplugged_dvs_scope_class))]
        assert_disk_serial(vm=fedora_vm_for_hotplug_scope_class, serials=expected_serials)
