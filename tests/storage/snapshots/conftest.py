...
from tests.storage.snapshots.constants import NUM_BLANK_DISKS, NUM_MULTI_DISK_VMS, WINDOWS_DIRECTORY_PATH
from tests.storage.snapshots.utils import (
    create_vm_with_4_disks,
    raise_cleanup_failures,
    restore_vm_within_deadline,
    run_parallel,
    wait_restore_within_deadline,
)
...
def vm_with_4_disks_for_snapshot(
    ...
    """Deploy and start a Fedora VM with four disk devices for snapshot restore testing.

    Yields:
        VirtualMachineForTests: Running VM with four disk devices and SSH connectivity.
    """
...
def restored_vm_with_4_disks(admin_client, vm_with_4_disks_for_snapshot, snapshot_of_vm_with_4_disks):
    ...
    with VirtualMachineRestore(
        ...
        wait_restore_within_deadline(restore=vm_restore, deadline=deadline)
        yield vm_restore
