...
class TestRestoreMultiDiskPerformance:
    ...
    def test_restore_vm_with_4_disks_completes_within_five_minutes(self, restored_vm_with_4_disks, vm_with_4_disks):
        """
        ...
        restored_vm = VirtualMachineForTests(
            name=restored_vm_with_4_disks.vm_name,
            namespace=restored_vm_with_4_disks.namespace,
            client=restored_vm_with_4_disks.client,
        )
    ...
    def test_restore_four_vms_with_4_disks_completes_within_five_minutes(
        ...
        _, disk_errors = run_parallel(
            ...
            func=lambda vm: (running_vm(vm=vm), assert_guest_disk_count(vm=vm)),
            label="Failed to verify restored VM disks",
            item_name=lambda vm: vm.name,
        )
        assert not disk_errors, f"Restored VMs failed disk verification: {', '.join(str(e) for e in disk_errors)}"
