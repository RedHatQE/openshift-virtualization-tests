...
def remaining_timeout(deadline: float, operation: str) -> float:
    ...
    if remaining <= 0:
        raise TimeoutError(f"{operation} timed out (deadline already passed)")
    return remaining
...
def start_restored_vm_and_assert_disk_count(vm: VirtualMachineForTests) -> None:
...
def start_restored_vms_and_assert_disk_counts(vms: list[VirtualMachineForTests]) -> None:
    ...
    _, disk_errors = run_parallel(
        items=vms,
        func=start_restored_vm_and_assert_disk_count,
