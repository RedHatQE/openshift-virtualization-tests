...
class TestNADReferenceLiveUpdate:
    ...
    Preconditions:
        ...
        - Workload update method of Kubevirt CRD set to LiveMigrate
        - Two NetworkAttachmentDefinitions (NAD1, NAD2) with two different bridges
        - One NetworkAttachmentDefinitions (NAD3) with the same name as NAD1 but in a different namespace (default/NAD1)
        - Running migratable VM with secondary network attached to NAD1
        - Running migratable VM with secondary network attached to NAD1
    ...
    def test_nad_ref_change_sets_correct_conditions(self):
        """
        Test that RestartRequired condition is not set and MigrationRequired is set and then removed
        when only NAD reference changes.
    ...
    def test_post_migration_isolation_from_previous_network(self):
        ...
        Preconditions:
            - VM(1) is now connected to NAD2 after NAD hotplug
    ...
    def test_guest_interface_mac_address_and_name_preserved(self):
        """
        Test that guest interface MAC address and name remains unchanged after NAD reference change.
        ...
        Expected:
            - MAC address and name remain unchanged
        """
    ...
    def test_post_nad_change_connectivity_to_new_network(self):
        ...
        Preconditions:
            - Running VM(1) with secondary network attached to NAD2 after NAD hotplug
            - Running VM(2) with secondary network attached to NAD2 after NAD hotplug
    ...
    @pytest.mark.polarion("CNV-XXXXX")
    def test_post_nad_change_migration(self):
        """
        Test that VM has network connectivity to the new network after NAD reference change.
    ...
    @pytest.mark.polarion("CNV-XXXXX")
    def test_multiple_nad_update(self):
        """
        Test NAD ref of more than one interface can be updated.

        Preconditions:
            - Running VM(1) with three secondary network attached to NAD1, NAD1 and NAD2
            - Running VM(2) with secondary network attached to NAD2

        Steps:
            1. Update VM2 spec to change multus.networkName of two ifaces from NAD1 to NAD2
            2. Wait for VirtualMachineInstanceMigrationRequired condition to appear and disappear
            3. Wait for VMI to be ready.
            4. Wait for AgentConnected condition on VMI.
            5. Execute ping from static VM2 to VM1 (both the interfaces)
    ...
    @pytest.mark.polarion("CNV-XXXXX")
    def test_updating_nad_of_different_namespace(self):
        """
        Test NAD ref can be updated when they belong to a different namespace.

        Preconditions:
            - Running VM(1) with three secondary network attached to NAD1, NAD2 and NAD3
            - Running VM(2) with secondary network attached to NAD2
        ...
        Steps:
            ...
            3. Wait for VMI to be ready.
            4. Wait for AgentConnected condition on VMI.
            5. Execute ping from static VM2 to VM1
