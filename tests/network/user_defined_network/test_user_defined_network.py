...
@pytest.fixture(scope="class")
def server_passt(vmb_passt: BaseVirtualMachine) -> Generator[TcpServer, None, None]:
    with TcpServer(vm=vmb_passt, port=SERVER_PORT) as server:
        yield server


@pytest.fixture(scope="class")
def client_passt(
    vma_passt: BaseVirtualMachine,
    vmb_passt: BaseVirtualMachine,
) -> Generator[VMTcpClient, None, None]:
    with TcpClient(
        vm=vma_passt,
        server_ip=lookup_iface_status(vm=vmb_passt, iface_name=lookup_primary_network(vm=vmb_passt).name)[IP_ADDRESS],
        server_port=SERVER_PORT,
    ) as client:
        yield client
...
@pytest.mark.ipv4
@pytest.mark.s390x
@pytest.mark.usefixtures("passt_binding")
