...
class TestVMWithExplicitIPAddressSpecification:
    ...
    def test_vm_is_started_with_successful_connectivity(
        ...
        netdata = cloudinit.NetworkData(
            ethernets={
                FIRST_GUEST_IFACE_NAME: cloudinit.EthernetDevice(
                    ...
                    gateway4=str(next(ipaddress.ip_network(ip_to_request, strict=False).hosts())),
                )
            }
        )
        ...
        assert read_guest_interface_ipv4(vm_under_test, interface_name=FIRST_GUEST_IFACE_NAME) == ip_to_request
