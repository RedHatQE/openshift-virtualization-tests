...
def hot_plug_interface(
    ...
    if is_jira_open(jira_id="CNV-96649"):
...
def set_secondary_static_ip_address(
    ...
    if is_jira_open(jira_id="CNV-96649"):
        ...
        LOGGER.warning(
            f"CNV-96649: Verified IP {hot_plugged_interface_ip} on {vmi_interface.name} via console "
...
def _lookup_hotplugged_iface_via_console(
    ...
    """Look up a hot-plugged interface via console when guest-agent is dead (CNV-96649).
    ...
    LOGGER.warning(
        f"CNV-96649: Guest agent did not report interface {spec_interface_name} on VM {vm.name}, "
    ...
    LOGGER.info(
        f"CNV-96649: looking for MAC {vmi_iface['macAddress']} in guest {vm.name}, visible interfaces: {visible_ifaces}"
    )
