...
pytestmark = pytest.mark.usefixtures(
    ...
    "grant_unprivileged_client_permission_for_migration",
)
...
def grant_unprivileged_client_permission_for_migration(admin_client, namespace):
    """Grant unprivileged_client permission for VM migration"""
    migration_role = ClusterRole(name="kubevirt.io:migrate", client=admin_client, ensure_exists=True)

    with RoleBinding(
        name="migrate_role_binding",
