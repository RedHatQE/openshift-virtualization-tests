...
@pytest.fixture()
def network_daemonset_deployment_resources(local_admin_client, hco_namespace):
    return [
        resource
        for _type in [DaemonSet, Deployment]
        for resource in _type.get(dyn_client=local_admin_client, namespace=hco_namespace.name)
