...
def compute_stuntime(source_vm: BaseVirtualMachine) -> float:
    ...
    result = source_vm.console(commands=[cmd_tail], timeout=10)
    if result is None:
        raise InsufficientStuntimeDataError(f"Failed to read ping log from VM {source_vm.name}")
...
class ContinuousPing:
    ...
    def _build_ping_cmd(self) -> str:
        ...
        """
        ping_ipv6_flag = _get_ping_ipv6_flag(destination_ip=self._destination_ip)
    ...
    def _verify_ping_reaches_destination(self) -> None:
        ...
        self._vm.console(
            commands=[build_ping_command(dst_ip=self._destination_ip, count=3, timeout=10)],
            timeout=15,
        )
