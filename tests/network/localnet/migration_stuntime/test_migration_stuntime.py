...
class TestMigrationStuntime:
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ...
                id="ipv6",
                marks=[
                    pytest.mark.polarion("CNV-15272"),
                    pytest.mark.ipv6,
                ],
            ),
        ],
    ...
    def test_client_migrates_off_server_node(
        ...
        measured = compute_stuntime(ping_log=stuntime_active_ping.stop_and_get_summary())
        assert measured <= STUNTIME_THRESHOLD_SECONDS, (
            f"Stuntime {measured}s exceeds threshold ({STUNTIME_THRESHOLD_SECONDS}s)"
        )
