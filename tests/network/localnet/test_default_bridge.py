...
def test_vmi_reports_ip_on_secondary_interface_without_vlan(
    ...
    if is_ipv6_single_stack_cluster():
        pytest.xfail(reason=f"{QUARANTINED}: The requested IP is assigned but not visible in VMI: CNV-80582")
    vm, _ = localnet_running_vms

    iface_status = lookup_iface_status(vm=vm, iface_name=LOCALNET_BR_EX_INTERFACE_NO_VLAN)
    reported_ips = set(
        filter_link_local_addresses(ip_addresses=[ip_interface(addr).ip for addr in iface_status.ipAddresses])
    )
