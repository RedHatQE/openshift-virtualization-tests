...
https://redhat.atlassian.net/browse/OCPBUGS-66994  # <skip-jira-utils-check>
...
from tests.network.localnet.liblocalnet import LOCALNET_BR_EX_INTERFACE
...
BEFORE_UPGRADE_TEST_ID = f"{os.path.abspath(__file__)}::test_connectivity_before_upgrade"
...
def test_connectivity_before_upgrade(subtests, localnet_running_vms):
    """
    Preconditions:
        - Two running VMs on different nodes with static IPs on localnet
    ...
    """
    vm_a, vm_b = localnet_running_vms
...
def test_connectivity_after_upgrade(subtests, localnet_running_vms):
    """
    Preconditions:
