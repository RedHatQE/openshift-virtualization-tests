...
def nad_localnet_ovs_bridge(
    ...
    with localnet_nad(
        namespace=namespace_localnet_1.name,
        name=f"{LOCALNET_OVS_BRIDGE_NETWORK}-nad",
        vlan_id=vlan_id,
...
def ovs_bridge_server_client_localnet_vms(
    vm1_ovs_bridge_localnet: BaseVirtualMachine, vm2_ovs_bridge_localnet: BaseVirtualMachine
) -> Generator[tuple[BaseVirtualMachine, BaseVirtualMachine]]:
    server_vm, client_vm = run_vms(vms=(vm1_ovs_bridge_localnet, vm2_ovs_bridge_localnet))
    yield (server_vm, client_vm)
