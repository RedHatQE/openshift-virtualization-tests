...
def test_connectivity_over_migration_between_ovs_bridge_localnet_vms(
    ...
    _, server, client = ovs_bridge_localnet_active_connections[0]
    migrate_vm_and_verify(vm=client.vm)
    for dst_ip, server, client in ovs_bridge_localnet_active_connections:
...
def test_connectivity_after_interface_state_change_in_ovs_bridge_localnet_vms(
    ...
    # Predicate also waits for expected IP addresses (IPv4/IPv6) to appear
