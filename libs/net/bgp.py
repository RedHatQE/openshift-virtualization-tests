...
@contextmanager
def enable_route_advertisements_in_cluster(network_resource: openshift_nc.Network) -> Generator[None]:
    """
    ...
    with ResourceEditor(patches=patch):
        deployment = Deployment(name=_FRR_DEPLOYMENT_NAME, namespace=_FRR_NS_NAME, client=cache_admin_client())
        deployment.wait_for_replicas()

        yield

    # The cleanup is expected by the un-patching of the changes (ResourceEditor cleanup).
    # However, it has been observed that the NS is not removed and therefore an explicit delete on the NS is performed.
    Namespace(name=_FRR_NS_NAME, client=cache_admin_client()).clean_up(wait=True)
