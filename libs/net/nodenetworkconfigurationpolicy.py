...
class NodeNetworkConfigurationPolicy(Resource):
    ...
    def __init__(
        ...
        Args:
            ...
            client (DynamicClient, optional): Optional DynamicClient to use.
    ...
    @retry(
        wait_timeout=WAIT_FOR_STATUS_TIMEOUT,
        sleep=WAIT_FOR_STATUS_SLEEP,
    )
    def wait_for_status_success(self) -> list[dict]:
        ...
        nncp_conditions = self.instance.status.conditions
        for condition in nncp_conditions:
            if condition["status"] == self.Condition.Status.TRUE:
                ...
                if (
                    condition["reason"] == no_match_node_condition_reason
                    or condition["reason"] == failed_condition_reason
                ):
                    raise NNCPConfigurationFailed(
                        f"{self.kind} {self.name} failed to configure on condition: {condition}\n"
                    )

        raise NNCPConfigurationFailed(f"{self.name}. None of the conditions were met.")

    def deploy(self, wait: bool = False) -> "NodeNetworkConfigurationPolicy":
    ...
    def _delete_interfaces(self) -> None:
        ...
        desired_state["interfaces"] = [
            ...
            if setattr(iface, "state", Resource.Interface.State.ABSENT) is None
        ]
    ...
    def interfaces(self) -> list[dict]:
        try:
            return self.instance.spec.desiredState.interfaces
        except KeyError:
            return []
